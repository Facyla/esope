...
function phpoffice_init() {
	...
	// Composer base
	$base_composer = elgg_get_plugins_path() . 'phpoffice/vendor/';
	...
	elgg_register_library('phpoffice:spreadsheet', $base . 'PhpSpreadsheet/vendor/autoload.php'); // manual install
	//elgg_register_library('phpoffice:spreadsheet', $base_composer . 'phpoffice/phpspreadsheet/src/Bootstrap.php'); // composer-installed
	elgg_register_library('elgg:phpoffice:spreadsheet', $base_lib . 'spreadsheet.php'); // custom lib
	...
	switch ($page[0]) {
		...
		*/
		...
		case 'excel':
			...
			case 'spreadsheet':
			elgg_load_library('phpoffice:spreadsheet');
			elgg_load_library('elgg:phpoffice:spreadsheet');
			if (($page[1] == 'samples') && (include $base . 'spreadsheet/' . $page[2] . '.php')) {
				// Samples are in pages/phpoffice/word
			} else if (empty($page[1]) || !include $excel_base . 'Examples/' . $page[1]) {
				include $base . 'spreadsheet.php';
			}
			break;
