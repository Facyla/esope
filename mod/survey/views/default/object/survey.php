...
if (!$survey instanceof ElggSurvey) { return; }
...
if ($full) {
	...
	//echo elgg_view('object/elements/full', array('entity' => $survey, 'summary' => $summary, 'icon' => $owner_icon));
	...
	// Add survey content : response form, or response message and responses (depending on survey settings)
	echo elgg_view('survey/survey_content', $vars);
	if ($survey->canEdit()) {
		echo '<a href="' . elgg_get_site_url() . 'survey/results/' . $survey->guid . '" class="elgg-button elgg-button-action">' . elgg_echo('survey:results') . '</a>';
	}
