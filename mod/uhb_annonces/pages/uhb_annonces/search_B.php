...
// We may also perform a cascading search to lower the "IN" clause scope at each iteration
// Note : each new clause has to include a filter based on previous clause results
// Note2 : @TODO use cascading by queryng at each level, and use previous result list, so we can break at any time
$md_filters = array();

function uhb_annonces_get_in_clause($metadata_name_value_pair, $values) {
...
if ($guid_filter && $search_params['metadata_name_value_pairs']) {
	$where_query = implode(',', $entity_filter);
	foreach($search_params['metadata_name_value_pairs'] as $md_filter) {
		...
		// Include previous SQL clause results into current clause filter
		$where_query = "$select $join WHERE $where AND md.entity_guid IN ($where_query)";
	}
} else { $where = false; }

$where = "WHERE md.entity_guid IN ($where_query)";
