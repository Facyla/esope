...
// Noter que cela a toutes les chances de faire planter le script, si on crée plus de 30 annonces à la fois (mais ne pose pas d'autre problème)
/*
if (false) {
	$offers = elgg_get_entities(array('types' => 'object', 'subtypes' => 'uhb_offer', 'limit' => 40));
	$all_fields = uhb_annonces_get_fields();
	echo count($offers) . " offres actuellement<br />";
	error_log(count($offers) . " offres actuellement");
	foreach ($offers as $ent) {
	set_time_limit(180);
		$new_ent = clone $ent;
		$new_ent->save();
#		$new_ent = new ElggObject;
#		$new_ent->subtype = 'uhb_offer';
#		$new_ent->owner_guid = $ent->owner_guid;
#		$new_ent->container_guid = $ent->container_guid;
#		$new_ent->access_id = $ent->access_id;
#		$new_ent->save();
#		foreach ($all_fields as $field) { $new_ent->$field = $ent->$field; }
		echo "OLD $ent->guid => NEW $new_ent->guid<br />";
		error_log("OLD $ent->guid => NEW $new_ent->guid");
	}
	exit;
}
*/
