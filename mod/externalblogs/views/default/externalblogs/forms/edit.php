...
$form_body .= 'URL du blog public&nbsp;: <a target="_new" href="' . elgg_get_site_url() . $entity->blogname . '">' . elgg_get_site_url() . $entity->blogname . '</a><br />';
$form_body .= '<a target="_new" href="' . elgg_get_site_url() . $entity->blogname . '?public=true">&raquo;&nbsp;Prévisualiser le blog (seuls les changements enregistrés sont visualisés)</a><br /><br />';
...
$form_body .= '<strong>URL du blog</strong> ' . elgg_get_site_url() . elgg_view('input/text', array('name' => 'blogname', 'value' => $entity->blogname, 'js' => ' style="width:200px;"')) . '<br /><br />';
...
$members = elgg_get_entities(array('types' => 'user', 'limit' => false));
...
$form_body .= 'Aide : liste des GUIDs des membres ' . $members_select . '<br /><br />';
...
$form_body .= elgg_view('input/select', array( 'name' => 'template', 'options_values' => $template_options, 'value' => $vars['entity']->template, 'js' => ' style="float:right;" onchange="javascript:$(\'.toggle_detail\').hide(); $(\'.toggle_detail.field_\'+this.value).show();"' ));
...
//if ($entity->template == 'zones') {
	...
	$form_body .= "Pages CMS disponibles : " . elgg_view('input/select', array('options_values' => $cmspages_options)) . '<br />';
	$form_body .= '<a target="_new" href="' . elgg_get_site_url() . 'cmspages">&raquo;&nbsp;Gérer les blocs via cmspages</a><br /><br />';
	// @TODO : remplacer le champ fulltext de configuration par un simple sélecteur de template
	// elgg_view('input/select', array('options_values' => $cmspages_options)
	// HOME
	$form_body .= '<div style="width:46%; float:left;">';
	$form_body .= elgg_view('input/select', array( 'name' => 'zones_home_layout', 'options_values' => $layout_options, 'value' => $vars['entity']->zones_home_layout, 'js' => ' style="float:right;"' ));
	...
	$form_body .= elgg_view('input/select', array( 'name' => 'zones_category_layout', 'options_values' => $layout_options, 'value' => $vars['entity']->zones_category_layout, 'js' => ' style="float:right;"' ));
	...
	// FULLVIEW
	$form_body .= '<div style="width:46%; float:left;">';
	$form_body .= elgg_view('input/select', array( 'name' => 'zones_fullview_layout', 'options_values' => $layout_options, 'value' => $vars['entity']->zones_fullview_layout, 'js' => ' style="float:right;"' ));
	...
	// PAGES CMS
	$form_body .= '<div style="width:46%; float:right;">';
	$form_body .= elgg_view('input/select', array( 'name' => 'zones_cmspages_layout', 'options_values' => $layout_options, 'value' => $vars['entity']->zones_cmspages_layout, 'js' => ' style="float:right;"' ));
	...
	// LISTING
	$form_body .= '<div style="width:46%; float:left;">';
	$form_body .= elgg_view('input/select', array( 'name' => 'zones_listing_layout', 'options_values' => $layout_options, 'value' => $vars['entity']->zones_listing_layout, 'js' => ' style="float:right;"' ));
...
// Compose form
echo elgg_view('input/form', array('action' => elgg_get_site_url() . 'action/externalblogs/edit', 'body' => $form_body));
