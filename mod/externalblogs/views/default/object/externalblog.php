...
if (!elgg_instanceof($externalblog, 'object', 'externalblog')) { return; }
...
if ($full && !elgg_in_context('gallery')) {
	// full view
	$body .= '<h3>' . $externalblog->title . ' (<a href="' . elgg_get_site_url() . $externalblog->blogname . '">' . elgg_get_site_url() . $externalblog->blogname . '</a>)</h3>';
	...
	if (elgg_is_logged_in() && $externalblog->canEdit()) {
		$body .= '<a href="' . elgg_get_site_url() . 'externalblog/edit/' . $externalblog->guid . '">Modifier</a>&nbsp; &nbsp;';
		$body .= elgg_view('output/url', array('href' => elgg_get_site_url() . 'action/externalblogs/delete?guid=' . $externalblog->guid, 'is_action' => true, 'text' => 'Supprimer')) . '<br />';
	}
