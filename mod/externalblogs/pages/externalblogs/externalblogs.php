...
foreach ($externalblogs as $externalblog) {
  if (elgg_is_logged_in()) {
    ...
    $content .= '<h3><a href="' . $CONFIG->url . $externalblog->blogname . '">' . $externalblog->title . ' (' . $CONFIG->url . $externalblog->blogname . '</a>)</h3><br />';
  }
  /*
  $content .= '<h3>' . $externalblog->title . ' (<a href="' . $CONFIG->url . $externalblog->blogname . '">' . $CONFIG->url . $externalblog->blogname . '</a>)</h3>';
  ...
  if (elgg_is_logged_in() && $externalblog->canEdit()) {
    $content .= '<a href="' . $CONFIG->url . 'externalblogs/edit/' . $externalblog->guid . '">Modifier</a>&nbsp; &nbsp;';
    $content .= elgg_view('output/url', array('href' => $CONFIG->url . 'action/externalblogs/delete?guid=' . $externalblog->guid, 'is_action' => true, 'text' => 'Supprimer')) . '<br />';
  }
  $content .= '<br /><br />';
  */
}
...
foreach ($externalblog_modules as $externalblog_module) {
  if (elgg_is_logged_in()) {
    ...
    $module_content .= '<h3><a href="' . $CONFIG->url . 'externalblog/module/' . $externalblog_module->guid . '">' . $externalblog_module->title . '</a></h3>' . $externalblog_module->description . '<br /><br />';
  }
}
