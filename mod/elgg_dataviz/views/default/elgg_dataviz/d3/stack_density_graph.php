...
$dataurl = $liburl . 'sdg_data.js';

//Parametrer hauteur et largeur
$width = elgg_extract('width', $vars, "600");
$height = elgg_extract('height', $vars, "400");
$svg_height = floor(($height-4)/2);
$svg_width = floor($width*.7);
$legend_width = floor($width*.28);
$legend_margin = floor($width*.02);
...
<style type="text/css">
#' . $id . ' {  }
#' . $id . ' svg { display:inline; float:left; margin-bottom:4px; }
#' . $id . ' .legend {
	...
	margin:0;
	margin-left:' . $legend_margin . 'px;
}
#' . $id . ' .legend table { border-collapse:collapse; border-spacing:0; }
#' . $id . ' .legend tr, #' . $id . ' .legend td, #' . $id . ' .legend div { margin:0; padding:0; }
#' . $id . ' .legend-filter { display: inline-block; margin: 0.1em; padding: 0.2em; }
#' . $id . ' .legend span { padding: 0 5px; margin:0; font-size:10px; }
#' . $id . ' .legend .legend-filter:hover { background:silver !important; }
#' . $id . ' .distquantdiv { clear:both; overflow:hidden; }
#' . $id . ' .distquantdiv h3 { margin-bottom:0; }
#' . $id . ' .stat { font-size:12px; }

#' . $id . ' path { fill-opacity:0.8; stroke-opacity:0; }
#' . $id . ' .vlines, #' . $id . ' .hlines { stroke:rgb(150,148,109); shape-rendering:crispEdges; stroke-width:1px; stroke-opacity:0.3; }
#' . $id . ' .hlabels line, #' . $id . ' .vlabels line {
	stroke:rgb(150,148,109);
	shape-rendering:crispEdges;
	stroke-width:1px;
	stroke-opacity:0.7;
}
#' . $id . ' .hlabels text, #' . $id . ' .vlabels text { color:rgb(150,148,109); font-size:12px; }
</style>
...
require(["elgg.dataviz.d3"], function(d3) {
	function distQuant(data, id){
		...
		var width=' . $svg_width . ', height=' . $svg_height . ', margin=0;
	...
	"#B1EDA3","#71D2E1","#A954DC","#91DF6E","#CB6429","#D64ADC"];
		...
		var legRow = d3.select("#"+id).append("div").attr("class","legend")
			...
			.selectAll("div").data(data.dP).enter();
		var legRowFilter = legRow.append("div").style("background",function(d,i){ return colors[i];}).attr("class","legend-filter")
			.on("mouseover",mouseoverLegend).on("mouseout",mouseoutLegend).style("cursor","pointer");

		legRowFilter.append("span").text(function(d){ return d[0];})
			.on("mouseover",mouseoverLegend).on("mouseout",mouseoutLegend).style("cursor","pointer");
	}
