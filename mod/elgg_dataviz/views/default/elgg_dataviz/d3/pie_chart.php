...
<style>
#' . $id . ' g.arc text { display: none; }
#' . $id . ' g.arc:hover text, #' . $id . ' g.arc:active text, #' . $id . ' g.arc:focus text { display: initial; }
#' . $id . ' .arc path { stroke: #fff; }
</style>
...
require(["elgg.dataviz.d3"], function(d3) {
	...
	d3.json("' . $dataurl . '", function(error, data) {
		...
		var focus = svg' . $elgg_d3js_unique_id . '.append("g")
			.attr("class", "focus")
			.style("display", "none");
		...
		/* Not functional
		svg' . $elgg_d3js_unique_id . '.append("rect")
			.attr("class", "overlay")
			.attr("width", width)
			.attr("height", height)
