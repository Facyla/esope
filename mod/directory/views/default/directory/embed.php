...
echo elgg_view("input/form", array("action" => "directory/embed/" . $object_guid, "id" => "directory-embed-search", "body" => $form_data));
...
if ($display) {
	...
	if ($count > 0) {
		...
		foreach ($entities as $entity) {
			...
			$embed_content .= " <strong>" . $entity->title . $entity->name . "</strong><br />";
			/*
			if ($entity->icontime) {
			...
			*/
			...
			echo '</div>';
			echo '<div class="directory-embed-item-content">' . $embed_content . '<br style="clear:both;" /></div>';
			echo '</li>';
		}
		echo '</ul>';
