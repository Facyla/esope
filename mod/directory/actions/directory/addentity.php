...
elgg_make_sticky_form('directory-addentity');

// Add new contribution (any member, if allowed by author)
if (!elgg_is_logged_in() || ($directory->write_access_id <= 0)) {
	register_error('directory:addentity:notallowed');
...
if ($entity_guid) {
	...
	if (in_array($entity_guid, $entities)) {
		register_error(elgg_echo('directory:addentity:alreadyexists'));
		forward(get_input('forward', REFERER));
	}
	...
	system_message(elgg_echo('directory:addentity:success'));
	elgg_clear_sticky_form('directory-addentity');
} else {
	register_error(elgg_echo('directory:addentity:emptyentity'));
}
