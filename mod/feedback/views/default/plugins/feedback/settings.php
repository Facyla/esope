<?php
$yesno_opt = ['no' => elgg_echo('option:no'), 'yes' => elgg_echo('option:yes')];
...
echo '<p><label>' . elgg_echo("feedback:settings:memberview") . elgg_view('input/select', ['name' => 'params[memberview]', 'options_values' => $yesno_opt, 'value' => $vars['entity']->memberview]) . '</label></p>';

// Are comments allowed on feedbacks ?
echo '<p><label>' . elgg_echo("feedback:settings:comment") . elgg_view('input/select', ['name' => 'params[comment]', 'options_values' => $yesno_opt, 'value' => $vars['entity']->comment]) . '</label></p>';

// Associate a group to feedbacks ?
echo '<p><label>' . elgg_echo("feedback:settings:feedbackgroup") . elgg_view('input/select', ['name' => 'params[feedbackgroup]', 'options_values' => $feedbackgroup_opt, 'value' => $vars['entity']->feedbackgroup]) . '</label></p>';


// Enable feedback types

// @TODO : Mood : global enable + available moods
echo '<p><label>' . elgg_echo("feedback:settings:enablemood") . elgg_view('input/select', ['name' => 'params[enable_mood]', 'options_values' => $yesno_opt, 'value' => $vars['entity']->enable_mood]) . '</label></p>';
/*
// Mood values
echo '<p><label>' . elgg_echo("feedback:settings:mood_values") . elgg_view('input/text', ['name' => 'params[mood_values]', 'value' => $vars['entity']->mood_values]) . '</label>';
echo '<br /><em>Default : happy, neutral, angry</em>';
*/

// @TODO : About : global enable
echo '<p><label>' . elgg_echo("feedback:settings:enableabout") . elgg_view('input/select', ['name' => 'params[enable_about]', 'options_values' => $yesno_opt, 'value' => $vars['entity']->enable_about]) . '</label></p>';
// About values
echo '<p><label>' . elgg_echo("feedback:settings:about_values") . elgg_view('input/text', ['name' => 'params[about_values]', 'value' => $vars['entity']->about_values]) . '</label>';
