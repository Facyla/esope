...
$base_url = elgg_get_site_url() . 'default_icons/icon';
...
// Nothing to hash provided: We display the form
if (empty($seed)) { 
	$content .= '<div style="font-family:Verdana,sans-serif; font-size:10pt;">VizHash_GD '.$VERSION.' - Visual Hash implementation in php4+GD.<br>';
	$example_url = $base_url.'?seed=hi';
	$content .= 'See <a href="http://sebsauvage.net/wiki/doku.php?id=php:vizhash_gd">homepage</a> for more details.';
	$content .= 'This is free software under the <a href="http://www.opensource.org/licenses/zlib-license.php">zlib/libpng licence</a><br>';
	$content .= '<form action="'.$base_url.'" method="GET">Enter string to hash: <input name="seed" id="seed" type="String to hash" size="100" value="" />';
	$content .= '<input id="ok" type="submit" value="Hash" /></form>';
	$content .= '<br>VizHash_GD accepts the following parameters:<br>';
	$content .= '<blockquote><b>t</b> : string to hash<br><b>width</b>: width of image (in pixels) (default:80, max:256)<br><b>height</b>: height of image (in pixels) (default:80, max:256)</blockquote><br>';
	$content .= 'Examples: "hi"';
	$content .= '<br>12x12:<br><img src="'.$example_url.'&width=12&height=12">';
	$content .= '<br>16x16:<br><img src="'.$example_url.'&width=16&height=16">';
	$content .= '<br>32x32:<br><img src="'.$example_url.'&width=32&height=32">';
	$content .= '<br>64x64:<br><img src="'.$example_url.'&width=64&height=64">';
	$content .= '<br>128x32:<br><img src="'.$example_url.'&width=128&height=32">';
	$content .= '<br>256x256:<br><img src="'.$example_url.'&width=256&height=256">';
	$content .= '</div>';
}

// We hash the input string. (We don't use hash() to stay compatible with php4.)
$hash=sha1($seed).md5($seed);
