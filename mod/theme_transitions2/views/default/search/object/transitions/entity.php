...
$entity_icon= elgg_view_entity_icon($entity, 'listing', array('align' => 'right'));

$body = '<p class="mbn">' . '<span class="transitions-category transitions-' . $entity->category . '" style="">' . elgg_echo('transitions:category:' . $entity->category) . '</span>' . $title . '</p>' . $description;
if ($extra_info) {
	$body .= "<p class=\"elgg-subtext\">$extra_info</p>";
}
$body .= "<p class=\"elgg-subtext\">$time</p>";

echo elgg_view_image_block($icon, $body, array('image_alt' => $entity_icon));
