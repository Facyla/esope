...
foreach ($vars['items'] as $menu_item) {
  // Adds title to links that do not have any (but filter ones that have an image or any other unreadable piece of code)
  /* Note : no use to have the same text as displayed => disabled
  if (empty($menu_item->title)) {
    $title = trim(strip_tags($menu_item->getText()));
    if (!empty($title)) $menu_item->setTooltip($title);
  }
  */
