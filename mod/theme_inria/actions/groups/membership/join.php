...
if ($user && ($group instanceof ElggGroup)) {
	...
	if ($join) {
		...
		// Notify group owner
		// Iris : notification is handled through create,relationship event handler, so no need to handle it here (would cause duplicate)
		// This is because request are made without using the join action, adn should also be notified (eg. external members invites)
