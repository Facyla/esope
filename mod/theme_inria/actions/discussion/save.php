...
if (empty($title)) {
	$topic->title = elgg_get_excerpt($desc, 120);
} else {
	$topic->title = $title;
}
...
if ($new_topic) {
	// If only description, forward to edit page instead
	if (empty($title)) {
		system_message(elgg_echo('theme_inria:discussion:quicksaved'));
		forward('discussion/edit/' . $topic->guid);
	}
