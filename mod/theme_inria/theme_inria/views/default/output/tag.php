...
/* Iris : force full search so we can filter on subtype
 *  Note : plus de recherche spécifique par tag : c'est désormais une option de filtrage, du coup si on cherche un type/subtype précis 
 * on bascule sur une recherche classique - mais dont les résultats incluent également les tags
*/
...
$query_params["search_type"] = "all";
$query_params["entity_subtype"] = "search_type:tags";


if (!empty($vars['type'])) {
	$query_params["type"] = $vars['type'];
	//$query_params["entity_type"] = $vars['type'];
	unset($vars['type']);
}

if (!empty($vars['subtype'])) {
	$query_params["subtype"] = $vars['subtype'];
	//$query_params["entity_subtype"] = $vars['subtype'];
	unset($vars['subtype']);
}
