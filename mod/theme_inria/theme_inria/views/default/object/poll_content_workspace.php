...
// Force results display
//$results_display = "block";
//echo elgg_view_form('poll/vote', array('id' => "poll-vote-form-workspace-{$poll->guid}"), array('entity' => $poll, 'callback' => 1));

?>

<div id="poll-post-body-workspace-<?php echo $poll->guid; ?>" class="poll_post_body" style="display:<?php echo $results_display ?>;">
...
if ($can_vote) {
	...
	if ($results_display != "block") {
		$toggle = elgg_view('output/url', array(
			'text' => $show_text,
			'href' => '',
			'data-guid' => 'workspace-'.$poll->guid,
