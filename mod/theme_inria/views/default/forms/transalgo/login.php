...
<div class="transalgo-login">
	...
	if ((current_page_url() == elgg_get_site_url().'transalgo') || (current_page_url() == elgg_get_site_url() . 'forgotpassword')) {
		echo '<div><a href="javascript:void(0);" onclick="$(\'#transalgo-lostpassword\').toggle(); $(\'#lostpassword_username\').val($(\'#login_username\').val()); $(\'.elgg-form-user-requestnewpassword input#username\').focus();" class="transalgo-lostpassword-toggle">' . elgg_echo('user:password:lost') . '</a></div>';
	} else {
		echo '<div><a href="' . elgg_get_site_url() . 'forgotpassword" class="transalgo-lostpassword" class="transalgo-lostpassword-toggle">' . elgg_echo('user:password:lost') . '</a></div>';
	}
