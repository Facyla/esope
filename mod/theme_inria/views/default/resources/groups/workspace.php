...
if (!$group->isMember()) {
	$add_content .= '<div class="group-workspace-add-tabs">';
		if ($is_main_group) {
			$add_content .= '<p><a href="' . $group->getURL() . '">' . elgg_echo('theme_inria:group:notmember') . '</a></p>';
		} else {
			$add_content .= '<p><a href="' . $group->getURL() . '">' . elgg_echo('theme_inria:workspace:notmember') . '</a></p>';
			// join - admins can always join.
			if ($group->isPublicMembership() || $group->canEdit()) {
				$add_content .= elgg_view('output/url', array(
...
// Membres : publication ssi autorisée dans ce groupe
if ($group->canWriteToContainer()) {
	...
	$add_content .= '<div class="group-workspace-add-tabs">';
		if ($group->thewire_enable == 'yes') {
			$add_content .= '<a href="#group-workspace-add-thewire" class="' . $selected . '" rel="nofollow" title="' . elgg_echo('theme_inria:workspace:addcontent:thewire') . '"><i class="fa fa-comment-o fa-fw"></i></a>';
			$selected = '';
		}
		//if ($group->forum_enable == 'yes') $add_content .= '<a href="#group-workspace-add-discussion" class="elgg-state-selected" rel="nofollow"><i class="fa fa-coments-o fa-fw"></i></a>';
		if ($group->blog_enable == 'yes') {
			$add_content .= '<a href="#group-workspace-add-blog" rel="nofollow" class="' . $selected . '" title="' . elgg_echo('theme_inria:workspace:addcontent:blog') . '"><i class="fa fa-file-text-o fa-fw"></i></a>';
			$selected = '';
		}
		//if ($group->file_enable == 'yes') {
		$add_content .= '<a href="#group-workspace-add-file" rel="nofollow" class="' . $selected . '" title="' . elgg_echo('theme_inria:workspace:addcontent:file') . '"><i class="fa fa-file fa-fw"></i></a>';
		//$selected = '';
		//}
	$add_content .= '</div>';
	
	// Publication tab contents (only first is displayed)
	$add_content .= '<div class="group-workspace-add-content">';
		$hidden = ''; // First tab that is displayed sets this to 'hidden'
		
		// The Wire
		if ($group->thewire_enable == 'yes') {
			$add_content .= '<div id="group-workspace-add-thewire" class="group-workspace-addcontent-tab ' . $hidden . '">';
				...
				$add_content .= elgg_view_image_block($own_image, $discussion_form);
			$add_content .= '</div>';
			$hidden = 'hidden';
		}
		...
		if ($group->blog_enable == 'yes') {
			$add_content .= '<div id="group-workspace-add-blog" class="group-workspace-addcontent-tab ' . $hidden . '">';
				// @TODO pour édition directe : on ajoute un début de texte puis on bascule sur le form complet pour finir d'éditer
				//$add_content .= elgg_view_form('blog/save');
				$add_content .= elgg_view('output/url', array(
		...
		if ($group->forum_enable == 'yes') {
			$add_content .= '<div id="group-workspace-add-discussion" class="group-workspace-addcontent-tab ' . $hidden . '">';
				$own_image = '<img src="' . $own->getIconURL(array('size' => 'small')) . '" />';
		...
		*/
		...
		//if ($group->file_enable == 'yes') {
			$add_content .= '<div id="group-workspace-add-file" class="group-workspace-addcontent-tab ' . $hidden . '">';
				/*
				$add_content .= elgg_view('output/url', array(
				...
				*/
				$add_content .= elgg_view_form('theme_inria/file/quick_upload', array('action' => 'action/file/upload', 'enctype' => "multipart/form-data"), array());
			$add_content .= '</div>';
			$hidden = 'hidden';
		//}
