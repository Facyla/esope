...
if (elgg_instanceof ($entity, 'user')) {
	...
	if (!$allowed) {
		...
		$icon = '<div class="elgg-avatar elgg-avatar-tiny profile-type-' . $profile_type . '"><a>' . elgg_view('output/img', array('src' => $spacer_url, 'style' => "background: url($icon_url) no-repeat;")) . '</a></div>';
...
if (!$icon) {
	...
	$type = $entity->getType();
	if ($type == 'user') {
		$icon = '<span class="elgg-avatar elgg-avatar-tiny profile-type-' . $profile_type . '"><img src="' . $entity->getIconUrl(array('size' => 'tiny')) . '" alt="' . $entity->name . '"></span>';
	} else if ($type == 'group') {
		...
		if (empty($profile_type)) { $profile_type = 'external'; }
		$icon = '<span class="elgg-avatar elgg-avatar-tiny profile-type-' . $profile_type . '"><img src="' . $owner->getIconUrl(array('size' => 'tiny')) . '" alt="' . $entity->name . '"></span>';
	} else {
