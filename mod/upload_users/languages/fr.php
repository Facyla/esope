...
$fr = array(
	...
	'upload_users:success' => "Le compte utilisateur a bien été créé succesfully",
	...
	'upload_users:upload_help' => "<p>Choisissez un fichier CSV et envoyez-le pour créer de nouveaux comptes utilisateurs. </p><p>La première ligne de ce fichier doit correspondre aux informations sur el contenu des colonnes. Les champs obligatoires sont : username, name, email (les intitulés doivent être ceux des noms internes de ces données dans Elgg - en anglais). Si le champ 'password' n'est pas défini, un mot de passe aléatoire sera généré. Si vous le souhaitez, vous pouvez envoyer automatiquement les informations de leur compte à chacun des nouveaux utilisateurs. </p><p>Vous pouvez ajouter autant de champs supplémentaires que vous le souhaitez. Tous ces cautres champs seront ajoutés aux informations du profil des membres. Si le délimiteur de votre fichier CSV est autre chose qu'une virgule ',', vous pouvez utiliser une virgule pour séparer les éléments d'une liste de tags</p><p>Voici un exemple de fichier CSV (attention, dans cet exemple la création du 1er compte ne fonctionnera pas car le mot de passe demandé est trop court ! minimum 6 caractères habituellement) :</p>",
