	...
	foreach ($providers as $key => $name) {
		...
		if (elgg_get_plugin_setting($key.'_enable', 'hybridauth') == 'yes') {
			if (elgg_is_logged_in()) {
				...
				$content .= '<a href="' . $CONFIG->url . 'hybridauth/' . $key . '" style="float:left; margin-right:0.5ex;" title="' . elgg_echo('hybridauth:loginwith', array($name)) . '"><img src="' . $CONFIG->url . 'mod/hybridauth/graphics/' . $key . '.png" alt="' . $name . '" /></a>';
			}
		}
	}
	$content .= '</p>';
