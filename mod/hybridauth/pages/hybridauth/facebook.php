...
try{
	...
	// automatically try to login with Facebook
	$facebook = $hybridauth->authenticate( "Facebook" );

	// return TRUE or False <= generally will be used to check if the user is connected to facebook before getting user profile, posting stuffs, etc..
	$is_user_logged_in = $facebook->isUserConnected();
	
	// User profile data
	$user_profile = $facebook->getUserProfile();
	$content .= "Ohai there! U are connected with: <b>{$facebook->id}</b><br />";
	...
	// User activity
	$user_activity = $facebook->getUserActivity();
	$content .= "User activity: <br /><pre>" . print_r( $user_activity, true ) . "</pre><br />";
	
	// Set status : post something to facebook if you want to
	// $facebook->setUserStatus( "Hello world!" );

	// ex. on how to access the facebook api with hybridauth
	//     Returns the current count of friends, followers, updates (statuses) and favorites of the authenticating user.
	//     https://dev.facebook.com/docs/api/1/get/account/totals
	//$account_totals = $facebook->api()->get( 'account/totals.json' ); // see facebook api doc
	//$content .= "Here some of yours stats on Facebook:<br /><pre>" . print_r( $account_totals, true ) . "</pre>";
