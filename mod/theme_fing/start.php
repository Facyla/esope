...
function theme_fing_init(){
	...
	// Extend on top so we can place it on 2 columns
	elgg_unextend_view('register/extend', 'forms/groups/register_join_groups', 0);
	elgg_extend_view('forms/register', 'forms/groups/register_join_groups', 0);
...
function fing_page_handler($page){
	...
	switch($page[0]){
		case 'ressources':
			...
			include(dirname(__FILE__) . '/pages/theme_fing/ressources.php');
			break;
		case 'archive':
		case 'projet':
		case 'prospective':
