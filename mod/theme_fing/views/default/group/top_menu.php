...
if (elgg_instanceof($group, 'group')) {
	...
	if (elgg_get_plugin_setting('groups_topmenu', 'adf_public_platform') == 'yes') {
		...
		if (!elgg_is_logged_in()) {
			echo '<div id="group-top-block">';
			$title_image = '<img src="' . $group->getIcon('large') . '" alt="' . $group->name . '" />';
			//$title_body = '<h1><a href="' . $group->getURL() . '" title="' . elgg_echo('esope:groups:home') . '">' . $group->name . '</a></h1>';
			$title_body = '<h1>' . $group->name . '</h1>';
			$title_body .= '<p><em>' . $group->briefdescription . '</em></p>';
			/*
			echo '<h1><a href="' . $group->getURL() . '" title="' . elgg_echo('esope:groups:home') . '">' . $image . $group->name . '</a></h1>';
			echo '<p><em>' . $group->briefdescription . '</em></p>';
			*/
			echo elgg_view_image_block($title_image, $title_body);
			echo '</div>';
			echo '<br />';
		}
