...
function forum_refresh() {
	...
	$('#loader').fadeIn('slow');
	$('#group-replies .elgg-list-entity').load('<?php echo $refresh_url . $parameters; ?>').fadeIn('fast');
	$('#loader').hide();
	...
	if (annotation_latest > 0) {
		$.ajax({
			...
			success: function(data) {
				$('#group-replies .elgg-list-entity').prepend(data);
			}
		});
	} else {
		$('#group-replies .elgg-list-entity').load('<?php echo $refresh_url . $parameters; ?>').fadeIn("slow");
	}
...
$(document).ready(function() {
	...
	$('#group-replies .elgg-list-entity').hide();
