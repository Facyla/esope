...
if (elgg_is_admin_logged_in()) {
	if (is_array($admin_exclude_access)) {
		foreach ($vars['options_values'] as $key => $val) {
			if (in_array($key, $admin_exclude_access) && !in_array($vars['name'], $donotmodify_cases)) { unset($vars['options_values'][$key]); }
		}
	}
} else {
	if (is_array($user_exclude_access)) {
		foreach ($vars['options_values'] as $key => $val) {
			if (in_array($key, $user_exclude_access) && !in_array($vars['name'], $donotmodify_cases)) { unset($vars['options_values'][$key]); }
		}
	}
}
