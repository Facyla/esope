...
/* Test URL examples:
 * Weekly site digest SITE_URL/digest/test/site/weekly
 * Daily group digest SITE_URL/digest/test/group/daily/4713
 */
...
digest_prepare_run();
// ESOPE: need to relocate views
$base = elgg_get_plugins_path() . 'digest';
elgg_set_view_location("icon/user/default", $base . "/views_alt/", "default");
elgg_set_view_location("river/elements/body", $base . "/views_alt/", "default");
...
$digest = "site";
$interval = get_input('interval', DIGEST_INTERVAL_MONTHLY);
if (!empty($page[2])) { $interval = $page[2]; }
...
switch ($page[1]) {
	case "group":
		if (!empty($page[3]) && ($group = get_entity($page[3])) && elgg_instanceof($group, "group")) {
			...
			register_error("Wrong URL: use site|group / interval / group guid"); // note : will not display if we don't include the messages view...
			forward('digest/test');
		}
	case "site":
	default:
