...
require_once $cmis_lib_path . 'include/CMISWebService.php';
require_once $cmis_lib_path . 'include/CMISAlfrescoSoapClient.php';
...
$url = 'https://partage.inria.fr:443/alfresco/cmisws/';
//$url = 'https://partage.inria.fr:443/alfresco/cmisws/';
...
$maxItems = 100;     // max number of documents on a page
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($action == 'login') {  // display login form
        $content .= $html->applyTemplate('loginForm');
        elgg_cmis_render_page($title, $content);
        exit;
    }
}
...
$content .= $html->applyTemplate('htmlHeader');
// all dialogs are included into the main page, to avoid page reloads for dialogs
$content .= createModalDialog($html, 'checkin', 'Checkin');
$content .= createModalDialog($html, 'checkout', 'Checkout');
$content .= createModalDialog($html, 'cancelCO', 'Cancel Checkout');
$content .= createModalDialog($html, 'uploadFile', 'Upload File');
$content .= createModalDialog($html, 'delete', 'Confirm Delete');
$content .= createModalDialog($html, 'createFolder', 'Create Folder');
$content .= createModalDialog($html, 'createDocument', 'Create Document');
$content .= $downloadFrame;    // frame used for file download
...
$content .= $html->applyTemplate('pageHeader', $errorMessage, $pccPath, $pccButtons);
...
function createModalDialog($html, $name, $title) {
    return $html->applyTemplate('modalDialog', $name, $title,
...
function elgg_cmis_render_page($title, $content) {
