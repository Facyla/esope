...
if ($repo_debug) {
		...
		$content .= '<pre>' . print_r($client->workspace, true) . '</pre>';
		$content .= "<br />===========================================<br /><br />";
}
...
if ($repo_debug) {
	$content .= '<br />' . "Folder Object:\n===========================================\n";
	$content .= print_r($myfolder, true);
	$content .= '<br />' . "\n===========================================\n\n";
}
...
/*
$repo_new_file_content = "THIS IS A NEW DOCUMENT";
$repo_new_file_mime = "text/plain";
$obs = $client->createDocument($myfolder->id, $repo_new_file, array (), $repo_new_file_content, $repo_new_file_mime);
if ($repo_debug) {
	$content .= '<br />' . "Return From Create Document\n:\n===========================================\n";
...
// Create new folder
$repo_new_folder = "Nouveau dossier personnalisé";
/*
$obs = $client->createFolder($myfolder->id, $repo_new_folder);
if ($repo_debug) {
	$content .= "Return From Create Folder\n:\n===========================================\n";
	$content .= print_r($objs, true);
	$content .= "\n===========================================\n\n";
}
...
// Delete file - OK : deleteObject($obj->id)
/*
$obj_del = $client->getObjectByPath($repo_folder . $repo_new_file);
$delContent = $client->getContentStream($obj_del->id);
$content .= "DELETING " . $obj_del->properties['cmis:name'] . "\n";
$client->deleteObject($obj_del->id);
*/

// Delete folder - OK : deleteObject($obj->id)
/*
$obj_del = $client->getObjectByPath($repo_folder . $repo_new_folder . '/');
$content .= "DELETING " . $obj_del->properties['cmis:name'] . "\n";
$client->deleteObject($obj_del->id);
*/

// Move file to folder - OK : moveObject($obj->id, $new_folder->id, $old_folder->id)
/*
$move_file = $client->getObjectByPath($repo_folder . $repo_new_file);
$sub_folder = $client->getObjectByPath($repo_folder . $repo_new_folder);
$client->moveObject($move_file->id, $sub_folder->id, $myfolder->id);
...
if ($repo_debug) {
	$content .= '<br />' . "Folder Children Objects\n:\n===========================================\n";
	print_r($objs);
	$content .= '<br />' . "\n===========================================\n\n";
}
foreach ($objs->objectList as $obj) {
	if ($obj->properties['cmis:baseTypeId'] == "cmis:document") {
		$content .= '<br />' . "Document: " . $obj->properties['cmis:name'] . "\n";
	} elseif ($obj->properties['cmis:baseTypeId'] == "cmis:folder") {
		$content .= '<br />' . "Folder: " . $obj->properties['cmis:name'] . "\n";
	} else {
		$content .= '<br />' . "Unknown Object Type: " . $obj->properties['cmis:name'] . "\n";
	}
}
