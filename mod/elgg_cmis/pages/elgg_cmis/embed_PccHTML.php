...
/*
global $CONFIG; $soap_url = $CONFIG->url . 'cmis';
const BASEURL = 'valeur constante';
self::BASEURL
//const BASEURL = 'cmis';
protected $baseurl = 'cmis'; 
*/
...
// recursive brackets are allowed
    ...
    private static $templates = array(
        // HTML Code sniplets
        'folderUrl' => '<a href="cmisembed?folderid={$1}">{$2}</a>',
        ...
        'docListEntry' => '<tr><td><a href="cmisembed?action=download&objectid={$1}">{$2}</a></td><td>{$3}</td><td align="right">{$4}</td></tr>',
        'docListEmpty' => "<tr><td>No documents in this folder</td></tr>\n",
        'pageLink' => '&nbsp;<a href="cmisembed?folderid={$1}&page={$2}">{$2}</a>&nbsp;',
        'pageCurrent' => '&nbsp;<b>{$1}</b>&nbsp;',
        'pageList' => '<div class="paging">{$1}</div>',
        'downloadFrame' => '<iframe width="1" height="1" frameborder="0" src="cmisembed?action=download&objectid={$1}"></iframe>',
        ...
        'detailsAction' => '<a href="cmisembed?action=details&objectid={$1}">More</a>&nbsp;',
...
<html lang='en_us'>
    ...
    <body>
        <form action="cmisembed" method="post">
...
,       'uploadFileDialog' => <<<EOF
    Select file to upload for document<div class="dialognotice"></div>
                <form id="{\$1}Form" method="POST" ENCTYPE="multipart/form-data" action="cmisembed">
                    <table><tbody><tr>
                        <td>File Name:</td><td>
...
,       'cancelCODialog' => <<<EOF
    Confirm Cancel Check-out for document<div class="dialognotice"></div>Unsaved changes will be lost!
                <form id="{\$1}Form" method="GET" action="cmisembed">
...
<html lang='en_us'>
    <head>
        ...
        <script src="mod/elgg_cmis/vendors/cmis/soap/include/dialog.js" type="text/javascript"></script>
        <title>PHP Alfresco CMIS Client</title>
        <link href="mod/elgg_cmis/vendors/cmis/soap/include/pcc.css" rel="stylesheet" type="text/css">
    </head>
    <body>
EOF
,       'htmlFooter' => <<<EOF
...
,       'pageHeader' => <<<EOF
        ...
        <div class="header">
            <span class="logout" style="float:right;"><a href="cmisembed?action=logout">Logout</a></span>
            {\$1}
