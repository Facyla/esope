...
if ($feed_url) {
	...
	if (!$num_posts_in_feed) {
		if (elgg_get_logged_in_user_guid() == elgg_get_page_owner_guid()) {
			echo '<p>' . elgg_echo('rss_simplepie:notfound') . '</p>';
		}
	}
	...
	foreach ($feed->get_items(0, $num_items) as $item) {
		...
		if ($post_date) {
			$item_date_label = elgg_echo('rss_simplepie:postedon');
...
HTML;
	...
	if (elgg_get_logged_in_user_guid() == elgg_get_page_owner_guid()) {
		echo '<p>' . elgg_echo('rss_simplepie:notset') . '</p>';
	}
}
