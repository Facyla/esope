 ...
 */
function videoembed_create_embed_object($url, $guid, $videowidth=0, $input = false) {
...
function videoembed_add_object($type, $url, $guid, $width, $height) {
	...
	switch ($type) {
    case 'facebook':
      $videodiv .= '<div id="fb-root"></div> <script>(function(d, s, id) { var js, fjs = d.getElementsByTagName(s)[0]; if (d.getElementById(id)) return; js = d.createElement(s); js.id = id; js.src = "//connect.facebook.net/nl_NL/all.js#xfbml=1"; fjs.parentNode.insertBefore(js, fjs); }(document, \'script\', \'facebook-jssdk\'));</script>';
...
function videoembed_dm_parse_url($url) {
	...
	if (!preg_match('/(https?:\/\/www\.dailymotion\.com\/.*\/)([0-9a-z]*)/', $url, $matches)) {
...
function videoembed_dm_parse_embed($url) {
	if (!preg_match('/(value="https?:\/\/)(www\.)?dailymotion\.com\/swf\/video\/([a-zA-Z0-9]*)/', $url, $matches)) {
