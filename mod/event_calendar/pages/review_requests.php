...
if (event_calendar_personal_can_manage($event,$user_id)) {
	...
	if ($requests) {
		...
		$body = elgg_view('page/elements/body',array('body'=>elgg_echo('event_calendar:review_requests_request_none')));
	}
} else {
	$body = elgg_view('page/elements/body',array('body'=>elgg_echo('event_calendar:review_requests_error')));
}
...
'sidebar' => '',
	...
	page_draw($title,elgg_view_layout("two_sidebar", $params));
