...
function event_calendar_get_entities_from_metadata_between_related($meta_start_name, $meta_end_name, 
	...
	$related_list = array();
	$related_events = elgg_get_entities_from_relationship(array("relationship" => "display_on_group", "relationship_guid" => $container_guid, "inverse_relationship" => TRUE));
	if ($related_events) {
...
function event_calendar_get_personal_events_for_user($user_id,$limit) {
	$events = elgg_get_entities_from_annotations(array("types" => "object", "subtypes" => "event_calendar", "annotation_names" => "personal_event", "annotation_values" => $user_id, "limit" => 1000));
	$final_events = array();
	if ($events) {
		$now = time();
		$one_day = 60*60*24;
...
function event_calendar_get_users_for_event($event_id,$limit,$offset,$is_count) {
	if ($is_count) {
		...
		$count = count_annotations($event_id, "object", "event_calendar", "personal_event");
                //echo $count;
                return $count;
	} else {
		$users = array();
...
function event_calendar_get_events_for_group($group_guid) {
	return elgg_get_entities(array('type' => 'object', 'subtype' => 'event_calendar', 'owner_guid' => 0, 'count' => false, 'site_guids' => 0, 'container_guids' => $group_guid));
}
