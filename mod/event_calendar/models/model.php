...
function event_calendar_activated_for_group($group) {
	// Facyla : patch suggested for quick workaround for group page_handler
	// Note : doesn't seem useful anymore ? @TODO check that before removing patch
...
// Added to support full calendar update - imported from new version
function event_calendar_get_page_content_fullcalendar_events($start_date,$end_date,$filter='all',$container_guid=0,$region='-') {
	...
	foreach($events as $e) {
		// Facyla : "if" quick patches to make it work on previous version
		if ($e['event']) $event = $e['event']; else $event = $e;
		if ($e['data']) $event_data = $e['data'];
		else $event_data = array(array('start_time' => $event->start_date, 'end_time' => $event->real_end_time));
		$c = count($event_data);
