...
if ($object = get_entity($guid)) {
  ...
  $pdf_filename = 'pdf_' . friendly_title($object->title) . '.pdf';
  ...
  $html .= '<br /><br /><br /><hr /><em>Cet article a été exporté automatiquement depuis la page ' . $object->getURL() . ' de FormaVia</em>';
} else {
...
$pdf->SetAuthor('FormaVia');
$pdf->SetTitle('TCPDF FormaVia export test');
$pdf->SetSubject('Test export PDF');
$pdf->SetKeywords('TCPDF, PDF, example, test, FormaVia');
